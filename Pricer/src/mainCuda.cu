...
__device__ float computeIterationGPU(float currentPrice, float h, int assetIndex, curandState* globalState, int sizeOpt, float* chol, float* sigma, float r){
  float scalarResult = 0.0;

  //Produit scalaire
  for(int i=0; i<sizeOpt;i++){
    scalarResult += chol[i + assetIndex*sizeOpt]*generate(globalState); //vectGauss[i];
  }
...
__device__ void assetGPU(float* path, int m, int n, float T, int N, int sizeOpt, float* spot, float* sigma, float* chol, float r, curandState* globalState){
  ...
  for(int i =1 ; i < N+1 ; i++){
    ...
    for(int j = 0 ; j < sizeOpt ; j ++){
      path[j + i*sizeOpt] = computeIterationGPU( path[j + (i-1)*sizeOpt], T/N, j, globalState, sizeOpt, chol, sigma, r);
    }
...
__global__ void priceGPU(float &prix, float &ic, float h, float H, int samples, int size, float r, float *trend,
                        float rho, float *spot, float *sigma, float *chol, float T, int timeSteps, float *payoffCoeff,
                        float strike, curandState* globalState) {
...
int main(int argc, char ** argv) {
  ...
  /*
  * Allocation du générateur aléatoire;
  */
  ...
  setup_kernel <<< 1, (mc->samples_) >>> ( devStates,unsigned(time(NULL)) );
